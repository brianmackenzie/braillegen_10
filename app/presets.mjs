...
export const PRESETS = {
  'ada': {
    ...
    note: 'Mid-range of ADA 2010 §703.3.1; dot height biased up for FDM printing.',
  },
  'california': {
 ...
 * @returns {{errors: string[], warnings: string[]}} errors block generation;
 * warnings are advisory.
 */
export function validateDimensions(v) {
  const errors = [];
  const warnings = [];
  const inRange = (key, label) => {
    const [lo, hi] = LIMITS[key];
    const val = v[key];
    if (!Number.isFinite(val)) errors.push(`${label} must be a number.`);
    else if (val < lo || val > hi) errors.push(`${label} must be between ${lo} and ${hi} mm.`);
  };
  ...
  if (v.dotPitch < v.dotDiameter + 0.3) {
    errors.push(`Dot pitch must be at least dot diameter + 0.3 mm (≥ ${(v.dotDiameter + 0.3).toFixed(2)} mm) or the dot bases merge.`);
  }
  if (v.cellPitch - v.dotPitch < v.dotPitch) {
    errors.push(`Cell pitch must be at least twice the dot pitch (≥ ${(2 * v.dotPitch).toFixed(2)} mm) so cells stay tactilely distinct.`);
  }
  if (linePitchToSpacing(v) < 0) {
    errors.push(`Line pitch is too small for this cell size (needs ≥ ${(2 * v.dotPitch + v.dotDiameter).toFixed(2)} mm).`);
  }
...
const STANDARDS = [
  ...
  {
    id: 'UKAAF / Marburg',
    dotDiameter: [1.25, 1.75], dotHeight: [0.4, 0.6], dotPitch: [2.4, 2.6],
    cellPitch: [5.9, 6.1], linePitch: [9.9, 10.1],
  },
];
